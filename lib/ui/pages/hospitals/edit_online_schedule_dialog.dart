...
class EditOnlineScheduleDialog extends StatefulWidget {
  ApiBuilderBloc bloc;
  GetHospitalTimingByDoctorHospital getDoctorTime;
  final DocID;

  EditOnlineScheduleDialog({Key? key,required this.getDoctorTime, this.DocID, required this.bloc}) : super(key: key);

  @override
  State<EditOnlineScheduleDialog> createState() =>
      _EditOnlineScheduleDialogState();
}

class _EditOnlineScheduleDialogState extends State<EditOnlineScheduleDialog> {
  List<String> selDay = [];
  List<String> selDays = [];
  ...
  @override
  void initState() {
    selDay=widget.getDoctorTime.day!.split(",");
    print("SSSSSSSSSSSS${widget.getDoctorTime.day?.split(",")}");
    fromTime.text=widget.getDoctorTime.fromTime!;
    toTime.text=widget.getDoctorTime.toTime!;
    print("${widget.getDoctorTime.fromTime}");
    print("${widget.getDoctorTime.toTime}");
    super.initState();
  ...
  Widget build(BuildContext context) {
    return MDialog(
      title: Row(
        children: [
          const Expanded(child: Text('EDIT ONLINE CONSULTATION SCHEDULE LIST')),
          IconButton(
              onPressed: () {
                context.popDialog();
              },
                ...
                onPressed: () async {
                  ...
                  if (fromTime.text.toString() == '' ||
                    ...
                    final res = await Injector()
                      ...
                      "Days": Days.toString(),
                      "VideoTiming_id": widget.getDoctorTime.videoTimingId,
